...
vec<Node*> dfs(Node* start, Node* end, vec<Node*> path) {
	if (*start == *end) {
		path.push_back(start);
		return path;
	}
	if (vec_contains(path, start)) { return {}; }

	vec<Node*> best;
	for (auto [n, d] : start->adj) {
		// cout << "(" << start->r << ", " << start->c << ") -> (" << n->r << ", " << n->c << ")" << endl;
		vec<Node*> new_path = path;
		new_path.push_back(start);
		vec<Node*> result = dfs(n, end, new_path);
		if (path_length(result) > path_length(best)) {
			best = result;
		}
	}
...
int main() {
	...
	vec<Node*> path = dfs(root, end, {});
	print_path(path);
	print_grid(-1, -1);
	cout << path_length(path) << endl;
	cout << len(path) << endl;
